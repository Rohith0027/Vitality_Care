...
<body> <input type="checkbox" id="menu">
    ...
    <div style="margin-top: 85px; width: 83.3%; height: 100vh; float: right;" class="filter">
        <div class="row">
            ...
            $id=$_SESSION['id'];
            $query = "SELECT * FROM `hospitalization` WHERE pid=$id";
            $query_run = mysqli_query($conn , $query);
            while($row = mysqli_fetch_array($query_run)){?>
            <div class="column">
                <div class="card">
                    <div class="container">
                        <h3>Doctor :<?php echo $row['dname'];?></h3>
                        <h4><b>Hospital :<?php echo $row['hname']; ?></b></h4>
                        <p>Issue :<?php echo $row['issue']; ?> (<?php echo $row['date']; ?>)</p>
                        <p>Price :<?php echo $row['prize']; ?></p>
                        <P>for queries contact - (<?php echo $row['phone']; ?>)</p>
                    </div>
                </div>
            </div>
            <?php } ?>
